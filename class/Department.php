	...
	public function listDepartment(){
		...
		while( $department = mysqli_fetch_assoc($result) ) {		
			...
			if($department['status'] == 1)	{
				$status = '<span class="label label-success">Habilitado</span>';
			} else if($department['status'] == 0) {
				$status = '<span class="label label-danger">Desabilitado</span>';
			}	
			...
			$departmentRows[] = '<button type="button" name="delete" id="'.$department["id"].'" class="btn btn-danger btn-xs delete">Deletar</button>';
