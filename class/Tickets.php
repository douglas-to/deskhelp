...
class Tickets extends Database {  
	...
	public function showTickets(){
		...
		while( $ticket = mysqli_fetch_assoc($result) ) {		
			...
			if($ticket['resolved'] == 0)	{
				...
				$status = '<span class="label label-danger">Fechado</span>';
			}	
			$title = $ticket['title'];
			...
			$ticketRows[] = '<button type="button" name="delete" id="'.$ticket["id"].'" class="btn btn-danger btn-xs delete"  '.$disbaled.'>Fechar</button>';
	...
	public function getRepliedTitle($title) {
		$title = $title.'<span class="answered">Respondidas</span>';
		return $title; 		
	}
